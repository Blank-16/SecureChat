...
export function ChatDashboard() {
  ...
  // Initialize E2E Chat & WebSocket hooks
    wsStatus,
    sendMessage,
    selectUser,
    loadHistory,
  ...
  // Setup typing debouncer to notify peer
  const { onInput, onStop } = useTypingDebounce((isTyping) => {
    if (selectedUser) {
      sendTyping(selectedUser, isTyping);
    }
  });
  ...
  // Trigger history decryptions as active conversation loads
  useEffect(() => {
    if (selectedUser) {
      loadHistory(selectedUser);
      clearUnread(selectedUser);
    }
  }, [selectedUser, activeMessages?.length, loadHistory, clearUnread]);

  // Scroll to bottom on new messages or typing animations
  useEffect(() => {
  ...
  async function handleSend(e: React.FormEvent) {
    ...
    onStop(); // Clear local typing state instantly
    setText("");
    const ok = await sendMessage(selectedUser, cleanText);
    if (!ok) {
      addToast("Failed to send message: encryption keys not fetched", "error");
    }
  }
  ...
  // Filter roster by search criteria
  const filteredContacts = useMemo(() => {
    return contacts
      .filter((u) => u.username !== currentUsername)
      .filter((u) => u.username.includes(search.toLowerCase()) || u.displayName.toLowerCase().includes(search.toLowerCase()));
  }, [contacts, currentUsername, search]);

  function handleAddContact() {
    const username = window.prompt("Enter exact username to add:");
    if (username && username.trim()) {
      addContact(username.trim());
    }
  }

  function handleDeleteChat() {
    if (selectedUser && window.confirm(`Delete entire chat with ${selectedUser}? This cannot be undone.`)) {
  ...
  function handleBlock() {
    if (selectedUser && window.confirm(`Block ${selectedUser}? You won't receive their messages.`)) {
  ...
  return (
    <div className="flex h-screen bg-surface-900 font-mono text-white select-none selection:bg-accent selection:text-white">
      {/* Container holding Sidebar and Chat area */}
      <div className="flex w-full max-w-7xl mx-auto border-2 border-surface-600 bg-surface-900 my-0 lg:my-4 h-full lg:h-[calc(100vh-2rem)] shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] rounded-none overflow-hidden">
        ...
        <div
          className={`
            ...
            lg:flex flex-col w-full lg:w-80 shrink-0 border-r-2 border-surface-600 bg-surface-800 z-30 h-full
          `}
        >
          {/* Header */}
          <div className="p-4 border-b-2 border-surface-600 flex justify-between items-center bg-surface-900">
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-sm tracking-tight truncate max-w-[120px] uppercase">
                {currentUsername}
              </span>
            </div>
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left p-4 flex items-center justify-between border-l-4 transition-all hover:bg-surface-700/40 cursor-pointer select-none rounded-none
                      ${
                        isSelected
                          ? "border-accent bg-surface-700/60"
        ...
        <div className="flex-1 flex flex-col bg-surface-900 h-full relative">
          ...
          {selectedUser ? (
              ...
              <div className="hidden lg:flex p-4 border-b-2 border-surface-600 items-center justify-between bg-surface-800">
                ...
                <div className="flex items-center gap-3">
                  <button
                    ...
                    className="text-[10px] font-bold px-2 py-0.5 border border-surface-600 text-surface-400 hover:text-red-400 hover:border-red-500 uppercase transition-colors"
                  >
                    DELETE_CHAT
                  </button>
                  <button
                  ...
                  <div className="h-full flex items-center justify-center flex-col text-center p-8 select-none">
                    <div className="border-2 border-surface-600 bg-surface-800 p-4 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] rounded-none">
                      <svg className="w-8 h-8 text-surface-500 mx-auto" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M8.678 20.89a12.094 12.094 0 01-1.996-2.228m1.996 2.228a12.074 12.074 0 001.815-4.07m-1.815 4.07c-.472-.503-.9-1.05-1.273-1.632m3.088-2.438c.452.44 1.018.808 1.666 1.077m-1.666-1.077a11.66 11.66 0 00-1.815-3.47m1.815 3.47a11.25 11.25 0 11-8.31-8.31m11.398 11.398a12.09 12.09 0 01-3.088-2.438m0 0a11.951 11.951 0 01-2.1-3.21m8.03-4.06a12.09 12.09 0 00-3.088-2.438m0 0a11.95 11.95 0 00-2.1-3.208m8.03 5.646a11.25 11.25 0 00-5.646-5.646l.002-.001z" />
                      </svg>
                      ...
                      <p className="text-[10px] text-surface-500 mt-1 uppercase max-w-xs leading-relaxed">
                        Say hello to {selectedUser}. All messages are symmetrically protected locally.
                      </p>
                    </div>
                  </div>
                ) : (
            ...
            /* Welcome Screen / Empty conversation state */
            <div className="flex-1 flex items-center justify-center p-8 bg-surface-900/60 text-center font-mono">
              <div className="max-w-md border-2 border-surface-600 bg-surface-800 p-8 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] rounded-none space-y-6">
                ...
                <p className="text-xs text-surface-400 leading-relaxed uppercase">
                  Select a user from the sidebar to establish a secure, end-to-end asymmetric cryptographic handshake channel.
                </p>
                ...
                <div className="border-t-2 border-surface-700/60 pt-4 flex flex-col gap-2 text-[10px] text-surface-500 text-left font-bold uppercase">
                  ...
                  <div>- Key Storage Protection: PBKDF2-AES-WRAP</div>
                </div>
              </div>
            </div>
          )}
